  ...
  ionViewWillEnter() {
    ...
    this.cartDataDisplay.forEach((element) => {
      ...
      this.service.forEach((element) => {
        let price = element.service_price * element.qty;
        element.service_price = price
        
        this.tot += element.total + price;
        this.api.total = this.tot;
        this.api.qty += element.qty;
        element.main_Total = element.qty * element.price + element.service_price
        localStorage.setItem("totalAmount", this.api.total);
      });
          

      localStorage.setItem("carFinal",JSON.stringify(this.cartDataDisplay))
    });
